        ...
        >
          ...
          <p className="text-gray-text-dim2 text-lg">Real patients, real results — inspiring journeys to better heart health</p>
                ...
                >
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            >
                                ...
                                <div className="w-full md:w-1/3 flex flex-col items-center text-center space-y-5 border-b md:border-b-0 md:border-r border-gray-100 pb-8 md:pb-0 md:pr-12">
                                    ...
                                    <div className="flex flex-wrap gap-2 justify-center text-brand-main ">
                    ...
                    <button onClick={prevSlide} className={`hidden md:flex absolute top-1/2 -translate-y-1/2 w-12 h-12 bg-white rounded-full items-center justify-center text-gray-text-dim2 transition-all z-20 hover:bg-brand-main hover:text-white border border-gray-100 cursor-pointer left-[-55px] ${currentIndex === 0 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                        <HiChevronLeft size={24} />
                    </button>
                    <button onClick={nextSlide} className={`hidden md:flex absolute top-1/2 -translate-y-1/2 w-12 h-12 bg-white rounded-full items-center justify-center text-gray-text-dim2 transition-all z-20 hover:bg-brand-main hover:text-white border border-gray-100 cursor-pointer right-[-55px] ${currentIndex === STORIES_DATA.length - 1 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                ...
                <div className="flex flex-col items-center mt-6 md:mt-10 space-y-4">
                    <div className="flex md:hidden items-center justify-center gap-6 mb-2">
                        <button onClick={prevSlide} className={`w-10 h-10 bg-white rounded-full flex items-center justify-center text-gray-text-dim2 transition-all hover:bg-brand-main hover:text-white border border-gray-200 cursor-pointer shadow-sm ${currentIndex === 0 ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                            <HiChevronLeft size={20} />
                        </button>
                        <button onClick={nextSlide} className={`w-10 h-10 bg-white rounded-full flex items-center justify-center text-gray-text-dim2 transition-all hover:bg-brand-main hover:text-white border border-gray-200 cursor-pointer shadow-sm ${currentIndex === STORIES_DATA.length - 1 ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                        {STORIES_DATA.map((_, i) => (
                            <div 
                                ...
                                className={`h-1.5 rounded-full cursor-pointer transition-all duration-300 w-[30px] md:w-[40px] ${currentIndex === i ? 'bg-brand-main' : 'bg-gray-200 hover:bg-gray-300'}`}
                            />
                        ))}
                    </div>
