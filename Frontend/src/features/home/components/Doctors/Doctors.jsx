...
>
  ...
  <p className="text-gray-text-dim2 text-base md:text-lg max-w-[850px] leading-[1.8] font-light">
    ...
    support for your heart health journey.
        ...
        >
          ...
          <button 
            onClick={prevSlide}
            className={`hidden md:flex absolute top-1/2 -translate-y-1/2 w-[54px] h-[54px] bg-white rounded-full items-center justify-center text-[#94a3b8] shadow-[0_10px_25px_rgba(0,0,0,0.06)] border border-[#f1f5f9] cursor-pointer z-30 transition-all duration-300 hover:bg-brand-main hover:text-white hover:shadow-[0_10px_20px_rgba(51,60,245,0.2)] left-[-20px] md:left-[-60px] ${currentIndex === 0 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
          >
            <HiChevronLeft size={24} />
          </button>

          {/* الكارد الرئيسي */}
          <div className="bg-white rounded-[32px] p-6 md:p-8 xl:p-14 shadow-lg min-h-[460px]">
                ...
                <div className="w-full xl:w-[45%] flex flex-col items-center xl:items-start text-center xl:text-left">
                  <div className="relative mb-6 xl:mb-8">
                    <img 
                      src={doctor.image} 
                      alt={doctor.name} 
                      className="w-32 h-32 md:w-40 md:h-40 xl:w-48 xl:h-48 rounded-full object-cover border-8 border-[#F8F9FA]" 
                    />
                 <div className="absolute bottom-3 right-3 xl:bottom-5 xl:right-5 w-4 h-4 bg-[#DCEDE8] border-2 xl:border-3 border-[#079A6B] rounded-full"></div></div>
                  ...
                  <p className="text-gray-text-dim2 text-xs md:text-sm mb-4 xl:mb-6">{doctor.exp} years</p>
  ...
  <FaUserGroup className="text-xs" /> {doctor.patients}
                ...
                <div className="w-full xl:w-[50%] mt-8 xl:mt-0">
                  ...
                  <div className="space-y-4 xl:space-y-5 mb-8 xl:mb-10 w-full max-w-sm mx-auto xl:max-w-none">
                    {doctor.metrics.map((m, i) => (
                      <div key={i} className="flex justify-between items-center border-b border-gray-50 pb-1.5 xl:pb-1">
                  ...
                  {/* ECG Box */}
                  <div className="bg-white rounded-2xl border border-gray-100 p-4 xl:p-5 w-full max-w-sm mx-auto xl:max-w-none">
          ...
          <div className="flex md:hidden items-center justify-center gap-6 mt-6">
            <button 
                onClick={prevSlide}
                className={`w-10 h-10 bg-white rounded-full flex items-center justify-center text-[#94a3b8] shadow-sm border border-[#f1f5f9] cursor-pointer transition-all duration-300 hover:bg-brand-main hover:text-white ${currentIndex === 0 ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}
              >
            ...
            <div className="flex justify-center gap-2">
              {DOCTORS.map((_, i) => (
                <button 
                  ...
                  className={`h-2 transition-all duration-500 rounded-full cursor-pointer ${currentIndex === i ? 'w-4 bg-brand-main' : 'w-2 bg-gray-200'}`}
                />
              ))}
            </div>
            ...
            <button 
                onClick={nextSlide}
                className={`w-10 h-10 bg-white rounded-full flex items-center justify-center text-[#94a3b8] shadow-sm border border-[#f1f5f9] cursor-pointer transition-all duration-300 hover:bg-brand-main hover:text-white ${currentIndex === DOCTORS.length - 1 ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}
              >
          ...
          <div className="hidden md:flex justify-center gap-2.5 mt-10">
            {DOCTORS.map((_, i) => (
              <button 
                key={i}
                onClick={() => setCurrentIndex(i)}
                className={`h-2.5 transition-all duration-500 rounded-full cursor-pointer ${currentIndex === i ? 'w-3 bg-brand-main' : 'w-2.5 bg-gray-200'}`}
              />
            ))}
          </div>
        </motion.div>
