    ...
    <section className="p-6 flex flex-col gap-6" aria-label="Activity Logs">
      ...
      <div className="flex flex-col gap-1 mb-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-[24px] sm:font-bold text-black-main-text leading-none">
            Activity Logs
          </h1>
        </div>
        <p className="text-[18px] text-gray-text-dim2 ">Track recent changes, updates, and system activity.</p>
      </div>

      {/* ── Controls ── */}
      <div className="flex flex-col sm:flex-row gap-3 p-4 sm:p-6 border border-[#F3F4F6] sm:rounded-full rounded-2xl">
        {/* Filter chips */}
        <div className="flex flex-wrap gap-2 ">
          {FILTERS.map(btn => (
            <button
              ...
              className={`px-4 sm:px-6 py-1 cursor-pointer text-[14px] font-normal rounded-full transition-colors filter-chip ${
                filter === btn
        ...
        {/* Search */}
        <div className="relative sm:ml-auto w-full sm:w-auto sm:hidden">
          ...
          <input
            ...
            className="w-full sm:w-[220px] pl-9 pr-4 py-2.5 text-[13px] bg-[#00000000] border border-gray-200 rounded-[10px] focus:outline-none focus:ring-2 focus:ring-[#155dfc]/30 focus:border-[#155dfc] transition-colors"
          />
        </div>
      </div>
                ...
                <li key={log.id} className="flex items-start gap-4 p-4  rounded-xl p-6 border border-[#F3F4F6] hover: transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 sm:gap-2">
                      ...
                      <span className="text-[14px] text-gray-text-dim2 shrink-0 whitespace-nowrap sm:text-right">{log.time}</span>
                    </div>
      ...
      {!isLoading && totalPages > 1 && (
        <div className="flex flex-wrap items-center justify-center gap-2 pt-1 m-6">
          <button
            disabled={currentPage === 1}
            onClick={() => setCurrentPage(p => p - 1)}
            className="cursor-pointer w-10 h-10 hidden sm:flex items-center justify-center rounded-full border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
