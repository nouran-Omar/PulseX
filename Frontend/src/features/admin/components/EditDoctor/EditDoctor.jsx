...
const FieldError = ({ msg }) =>
  msg ? (
    <span className="flex items-center gap-1.5 text-[14px] font-normal text-[#DC2626] mt-1">
...
export default function EditDoctor() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      navigate('/admin/doctor-management', { state: { success: true, title: 'Updated Successfully' } });
    },
  });
  ...
  const handleDeleteConfirm = () => {
    ...
    navigate('/admin/doctor-management', { state: { success: true, title: 'Deleted' } });
  };
    ...
    <section className="bg-white  overflow-hidden min-h-screen">
      ...
      <form onSubmit={formik.handleSubmit} className="py-8 px-4 md:py-12 md:pl-8 md:pr-8 lg:pr-[145px]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] xl:grid-cols-[280px_1fr] gap-8 md:gap-12">
          ...
          <div className="bg-[#F9FAFB] border border-gray-200 rounded-[24px] p-5 md:p-8 shadow-sm h-fit">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              <InputField label="Email Address" name="email" icon={HiOutlineEnvelope} formik={formik} placeholder="doctor@pulsex.com" />
              ...
              <InputField label="Location" name="location" icon={HiOutlineMapPin} formik={formik} placeholder="City, Country" />
              <InputField label="Consultation Price" name="price" type="number" icon={HiOutlineCurrencyDollar} formik={formik} placeholder="0.00" />
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-center sm:justify-end gap-3 md:gap-4 mt-8 md:mt-12">
          <button type="button" onClick={() => navigate('/admin/doctor-management')} className="w-full sm:w-auto px-8 md:px-12 py-3 md:py-3.5 rounded-full bg-[#E4E7EC] text-[#344054] text-[15px] md:text-[16px] font-normal hover:bg-gray-300 transition-all cursor-pointer">
            Cancel
          </button>
