  ...
  return (
    ...
    <div className="relative flex flex-col gap-4">
      ...
      {selectedItems.length > 0 && (
        <div className="absolute top-[70px] left-1/2 -translate-x-1/2 z-20 flex w-max max-w-[95vw] flex-wrap justify-center items-center gap-3 px-3 py-2 bg-white border border-gray-100 rounded-xl shadow-[0px_10px_30px_rgba(0,0,0,0.15)] animate-in fade-in slide-in-from-top-4 duration-300">
      ...
      {/* Table Container */}
      <div className="overflow-visible sm:overflow-x-auto rounded-2xl sm:rounded-xl border-0 sm:border sm:border-gray-200/70 bg-white my-3">
          ...
          <tbody>
            {currentItems.map((item, idx) => {
              ...
              return (
                <tr 
                  key={item.id} 
                  className={`block sm:table-row rounded-2xl sm:rounded-none overflow-hidden border border-slate-200/80 sm:border-b sm:border-gray-100 shadow-[0_6px_18px_rgba(15,23,42,0.08)] sm:shadow-none mb-4 sm:mb-0 transition-colors text-center ${
                    isSelected ? 'bg-[#EFF6FF]' : idx % 2 === 0 ? 'sm:bg-white bg-white' : 'sm:bg-[#F9FAFB] bg-white'
                  } hover:bg-[#EFF6FF]/60`}
                >
                  ...
                  <td className="block sm:table-cell px-4 py-3 text-center sm:text-left">
                    <div className="flex flex-row items-center justify-center sm:justify-start gap-3 rounded-xl bg-[#F7F7FA] px-3 py-2 sm:bg-transparent sm:px-0 sm:py-0">
                      ...
                      />
                      <span className="text-[16px] font-semibold text-black-main-text truncate max-w-[170px]">
                        {item.fullName}
                      </span>
                    </div>
                  </td>
                  <td className="block sm:table-cell px-4 py-3 text-center">
                    <span className="text-[16px] text-gray-text-dim2 truncate block max-w-[220px] mx-auto">
                      {item.email}
                    </span>
                  </td>
                  <td className="block sm:table-cell px-4 py-3 text-center">
