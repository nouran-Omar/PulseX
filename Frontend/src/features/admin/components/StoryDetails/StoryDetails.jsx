...
export default function StoryDetails() {
  ...
  if (!story) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 px-4 py-20 sm:px-6">
  ...
  return (
    <section className="flex flex-col gap-6 p-4 sm:p-5 lg:p-6" aria-label="Story Details">
  ...
  <path d="M26 1.78694C26.1796 1.89062 26.3325 2.03475 26.4466 2.20788C26.5607 2.38101 26.6329 2.57837 26.6573 2.78428L26.6667 2.94161V20.2749C26.6667 20.509 26.605 20.7389 26.488 20.9416C26.371 21.1443 26.2027 21.3126 26 21.4296C25.7973 21.5466 25.5674 21.6082 25.3333 21.6082C25.0993 21.6082 24.8694 21.5466 24.6667 21.4296C23.1559 20.5573 21.4543 20.0688 19.7108 20.0069C17.9674 19.9451 16.2354 20.3118 14.6667 21.0749V0.842943C16.5085 0.154954 18.4797 -0.117187 20.439 0.0460152C22.3984 0.209217 24.2974 0.803721 26 1.78694ZM12 0.844276L12.0013 21.0763C10.4944 20.3427 8.83581 19.9745 7.16002 20.0016C5.48424 20.0287 3.83842 20.4503 2.356 21.2323L1.92 21.4723L1.78267 21.5309L1.71733 21.5523L1.57067 21.5869L1.48933 21.6003L1.33333 21.6083H1.27733L1.13067 21.5923L1.028 21.5736L0.884 21.5309L0.716 21.4563L0.589333 21.3816L0.470667 21.2923L0.390667 21.2176L0.293333 21.1083L0.208 20.9896L0.178667 20.9416L0.136 20.8616L0.0773335 20.7243L0.0560001 20.6589L0.0213334 20.5123L0.00800006 20.4309L0.00266679 20.3656L0 2.94161C5.94563e-06 2.70757 0.0616172 2.47765 0.178641 2.27496C0.295665 2.07228 0.463979 1.90396 0.666667 1.78694C2.3694 0.803939 4.26845 0.209667 6.22779 0.0466955C8.18713 -0.116276 10.1583 0.156088 12 0.844276Z" fill="#010218"/>
      ...
      {/* Author Card */}
      <div className="flex flex-col sm:flex-row items-start gap-4 p-4 sm:p-5 shadow-[0px_2px_3px_0px_rgba(0,0,0,0.10)] outline-1 outline-offset-[-1px] outline-gray-100 rounded-[16px]">
        ...
        <span className={`text-[12px] font-bold px-2.5 py-1 rounded-full shrink-0 self-start sm:self-auto ${statusClass}`}>{story.status}</span>
      </div>

      {/* Story Content */}
      <div className="bg-white border border-gray-100 rounded-[16px] p-4 sm:p-5 lg:p-6 flex flex-col gap-4">
      ...
      {/* Engagement Bar */}
      <div className="flex flex-wrap items-center gap-3 sm:gap-6 px-4 sm:px-5 py-3.5 rounded-[14px]">
      ...
      {/* Comments Preview */}
      <div className="bg-white border border-gray-100 rounded-[16px] p-4 sm:p-5 flex flex-col gap-4">
        ...
        <ul className="flex flex-col gap-3 list-none p-0 m-0">
          {previewComments.length === 0 ? (
            ...
            previewComments.map(comment => (
              <li key={comment.id} className="flex items-start gap-3 py-3 bg-gray-50 rounded-2xl outline-[0.80px] outline-offset-[-0.80px] outline-gray-200 p-4 sm:p-5 last:border-0">
                {comment.avatar ? (
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white rounded-3xl shadow-[0px_2px_3px_0px_rgba(0,0,0,0.10)] outline-1 outline-offset-[-1px] p-4 sm:p-5 outline-gray-100 pt-2">
        <button onClick={() => navigate('/admin/stories-management')} className="flex items-center justify-center sm:justify-start gap-2 px-4 py-2 text-[14px] font-semibold cursor-pointer text-black-main-text">
          <HiArrowLeft /> Back to Stories
        </button>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <button onClick={() => setHideModal(true)} disabled={story.status === 'Deleted'} className="flex items-center justify-center gap-2 px-4 py-2 text-[14px] font-semibold text-black-main-text bg-gray-200 rounded-full cursor-pointer hover:bg-gray-300 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
            {story.status === 'Hidden' ? <><HiOutlineEye /> Unhide Story</> : <><HiOutlineEyeSlash /> Hide Story</>}
          </button>
          <button onClick={() => setDeleteModal(true)} disabled={story.status === 'Deleted'} className="flex items-center justify-center gap-2 px-4 py-2 text-[14px] font-semibold text-white bg-red-700 rounded-full cursor-pointer hover:bg-red-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
