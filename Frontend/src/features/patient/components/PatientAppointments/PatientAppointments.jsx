...
const PatientAppointments = () => {
  ...
  return (
    <section className="flex flex-col gap-6 p-4 sm:p-[24px]">
      <ConfirmModal
        isOpen={!!cancelTarget}
  ...
  <p className="text-[18px] text-[#757575] max-w-2xl pl-[8px]">
    View your scheduled and completed appointments
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
        <div className="bg-white rounded-[20px] border border-green-100 shadow-sm p-5 sm:p-7 flex flex-col sm:flex-row items-center sm:items-start lg:items-center gap-4 text-center sm:text-left transition-all hover:shadow-md">
          <div className="w-[60px] h-[60px] sm:w-[80px] sm:h-[80px] rounded-full  flex items-center justify-center shrink-0"
          
          style={{background: 'linear-gradient(201deg, #D3FFE7 0%, #EFFFF6 100%)'}}
          
          >
            < LuCalendarClock className="text-[#00AC4F] text-[32px] sm:text-5xl" />
          </div>
          <div className="flex flex-col gap-1 sm:gap-0 mt-2 sm:mt-0">
            <p className="text-[16px] text-[#757575]">Upcoming Appointments</p>
            <h3 className="text-[32px] font-bold text-brand-main leading-none">{upcoming.length}</h3>
          </div>
        </div>
        <div className="bg-white rounded-[20px] border border-indigo-100 shadow-sm p-5 sm:p-7 flex flex-col sm:flex-row items-center sm:items-start lg:items-center gap-4 text-center sm:text-left transition-all hover:shadow-md">
          <div className="w-[60px] h-[60px] sm:w-[80px] sm:h-[80px] rounded-full bg-indigo-50 flex items-center justify-center shrink-0"
          
          style={{background: 'linear-gradient(201deg, rgba(51, 60, 245, 0.65) 0%, rgba(51, 60, 245, 0.05) 100%)'}}
          >
            <GiCheckMark className="text-brand-main text-[21px] sm:text-[32px]" />
          </div>
          <div className="flex flex-col gap-1 sm:gap-0 mt-2 sm:mt-0">
            <p className="text-[16px] text-[#757575]">Completed Appointments</p>
            <h3 className="text-[32px] sm:text-[23px] font-bold text-brand-main leading-none">{completed.length}</h3>
          </div>
        </div>
      </div>
  ...
  >
    Completed
      ...
      <div className="flex flex-col gap-4">
        {list.length === 0 && (
          <p className="text-[16px] text-[#757575] text-center py-8">No {activeTab} appointments.</p>
        )}
        {list.map(app => (
          <div key={app.id} className="bg-white rounded-[20px] border border-gray-100 shadow-sm p-4 sm:p-5 flex flex-col md:flex-row items-center md:items-center justify-between gap-5 transition-all hover:shadow-md">
            <div className="flex flex-col sm:flex-row items-center sm:items-start md:items-center gap-4 w-full md:w-auto text-center sm:text-left">
              <img src={app.img} alt={app.doc} className="w-20 h-20 sm:w-16 sm:h-16 md:w-14 md:h-14 rounded-full object-cover border-2 border-white shadow-sm shrink-0" />
              <div className="flex flex-col gap-2 sm:gap-1.5 w-full">
                ...
                <div className="flex flex-col sm:flex-row flex-wrap items-center sm:items-start justify-center sm:justify-start gap-2 sm:gap-4 w-full">
                  <div className="flex items-center gap-1.5 text-[16px] text-[#757575] bg-gray-50 sm:bg-transparent px-3 py-1.5 sm:px-0 sm:py-0 rounded-lg shrink-0">
                ...
                <div className="flex items-center gap-1.5 text-[16px] text-[#757575] bg-gray-50 sm:bg-transparent px-3 py-1.5 sm:px-0 sm:py-0 rounded-lg w-fit mx-auto sm:mx-0 shrink-0">
            ...
            <div className="w-full md:w-auto mt-2 md:mt-0 flex justify-center md:justify-end border-t border-gray-100 md:border-none pt-4 md:pt-0 shrink-0">
              {activeTab === 'upcoming' ? (
                ...
                >
                  <HiOutlineXCircle className="text-[20px]" /> Cancel Appointment
                </button>
              ) : (
