...
/* ─────────────────────────────────────────────────────────────
   Animated Filter Panel — smooth height + fade + stagger
───────────────────────────────────────────────────────────── */
...
const AnimatedFilterPanel = ({ open, filter, onSelect }) => {
  ...
  /* measure real height once content is rendered */
  useEffect(() => {
    if (open) {
      setMounted(true);
  ...
  const getTabClass = (opt) => {
    ...
    switch (opt) {
      case 'Active Only':    return base + 'bg-[#00A63E] text-white shadow-md scale-[1.04]';
      case 'Completed Only': return base + 'bg-[#4A5565] text-white shadow-md scale-[1.04]';
      default:               return base + 'bg-brand-main text-white shadow-md scale-[1.04]';
    }
  };
    ...
    >
      ...
      <div className="bg-white rounded-2xl px-5 py-4 mb-4 shadow-sm flex flex-col gap-3 transition-all duration-500 ease-in-out">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          {/* Filter Button with animated arrow */}
          <button
