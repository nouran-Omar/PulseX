...
const LockIcon = () => (
  <svg
    ...
    fill="none"
    role="img"
    aria-label="Secure payment lock"
    className="text-[var(--payment-brand)]"
  >
      ...
      <section className="fixed inset-0 flex items-center justify-center z-[100] bg-black/40 backdrop-blur-sm p-4" aria-live="polite">
        <div className="bg-white rounded-[32px] p-10 max-w-sm w-full text-center shadow-2xl">
          <div className="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-6">
            <HiCheckCircle className="text-emerald-500 text-5xl" aria-hidden="true" />
          </div>
          ...
          <button
            ...
            className="w-full py-4 bg-[var(--payment-brand)] text-white rounded-2xl font-bold hover:bg-[var(--payment-brand-hover)] transition-all"
          >
            Done
          </button>
        </div>
  ...
  return (
    <section className="max-w-6xl mx-auto">
      <header className="flex items-center gap-3 mb-10">
        <div>
          <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40" fill="none" role="img" aria-label="Payment method icon">
  ...
  <defs>
    <clipPath id="clip0_1770_12705">
      <rect width="40" height="40" fill="white"/>
        ...
        <h1 className="text-2xl font-bold text-[var(--payment-ink)]">Select Payment Method</h1>
      </header>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <article className="lg:col-span-7 space-y-6">
          <div 
            ...
            className={`bg-white p-6 rounded-[24px] border-2 cursor-pointer flex items-center justify-between transition-all ${method === 'cash' ? 'border-[var(--payment-brand)]' : 'border-gray-200'}`}
          >
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-100">
                <span className="text-xl" role="img" aria-label="Cash payment icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
<g opacity="0.6">
...
<path d="M2 10H22" stroke="#333CF5" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              ...
              <div>
                <p className="font-bold text-[var(--payment-ink)]">Cash at Clinic</p>
                <p className="text-sm text-slate-400">Pay directly at the clinic during your visit</p>
              </div>
            </div>
            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${method === 'cash' ? 'border-[var(--payment-brand)]' : 'border-gray-200'}`}>
              {method === 'cash' && <div className="w-3 h-3 bg-[var(--payment-brand)] rounded-full" />}
            </div>
          </div>

          <div 
            onClick={() => setMethod('credit')}
            className={`bg-white p-6 rounded-[24px] border-2 cursor-pointer flex items-center justify-between transition-all ${method === 'credit' ? 'border-[var(--payment-brand)]' : 'border-gray-200'}`}
          >
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-100">
                <span className="text-xl" role="img" aria-label="Card payment icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <g opacity="0.6">
    ...
    <path d="M3 5V19C3 19.5304 3.21071 20.0391 3.58579 20.4142C3.96086 20.7893 4.46957 21 5 21H20C20.2652 21 20.5196 20.8946 20.7071 20.7071C20.8946 20.5196 21 20.2652 21 20V16" stroke="#0A0A0A" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              ...
              <div>
                <p className="font-bold text-[var(--payment-ink)]">Credit / Debit Card</p>
                <p className="text-sm text-slate-400">Pay securely with your credit or debit card</p>
              </div>
            </div>
            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${method === 'credit' ? 'border-[var(--payment-brand)]' : 'border-slate-200'}`}>
              {method === 'credit' && <div className="w-3 h-3 bg-[var(--payment-brand)] rounded-full" />}
            </div>
          </div>

          {method === 'credit' && (
            <section className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100 space-y-6 animate-in fade-in duration-500" aria-label="Card details">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" placeholder="John Doe" className="w-full p-4 rounded-xl border border-slate-100 bg-white outline-none focus:border-[var(--payment-brand)] transition-all text-slate-700" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-slate-500 mb-2">Card Number</label>
                  <input type="text" placeholder="1234 5678 9012 3456" className="w-full p-4 rounded-xl border border-slate-100 bg-white outline-none focus:border-[var(--payment-brand)] transition-all text-slate-700 tracking-wider" />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-500 mb-2">Expiry Date</label>
                    <input type="text" placeholder="MM/YY" className="w-full p-4 rounded-xl border border-slate-100 bg-white outline-none focus:border-[var(--payment-brand)] transition-all" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-500 mb-2">CVV</label>
                    <input type="text" placeholder="123" className="w-full p-4 rounded-xl border border-slate-100 bg-white outline-none focus:border-[var(--payment-brand)] transition-all" />
                  </div>
                </div>
              </div>
        ...
        <aside className="lg:col-span-5" aria-label="Booking summary">
          <div className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100">
            <h2 className="text-lg font-bold text-[var(--payment-ink)] mb-8">Booking Summary</h2>

            <div className="space-y-6">
              <SummaryRow icon={<HiOutlineUser className="text-blue-500" aria-hidden="true" />} label="Doctor" iconBg="bg-blue-50" iconLabel="Doctor">
              ...
              <SummaryRow icon={<HiOutlineCalendar className="text-slate-500" aria-hidden="true" />} label="Date" iconLabel="Appointment date">
                <p className="font-bold text-slate-800 text-sm">{bookingData.date}</p>
              </SummaryRow>

              <SummaryRow icon={<HiOutlineClock className="text-slate-500" aria-hidden="true" />} label="Time" iconLabel="Appointment time">
                <p className="font-bold text-slate-800 text-sm">{bookingData.time}</p>
              </SummaryRow>

              <SummaryRow icon={<span className="text-slate-500" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
<g opacity="0.6">
...
<path d="M1.66797 8.3335H18.3346" stroke="#010218" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/>
              ...
              </svg></span>} label="Payment Method" iconLabel="Payment method">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-[var(--payment-ink)]">${bookingData.price}.00</span>
              </div>

              <button 
                onClick={() => setSuccess(true)}
                className="w-full py-4 cursor-pointer bg-[var(--payment-brand)] text-white rounded-2xl font-bold text-base hover:bg-[var(--payment-brand-hover)] transition-all shadow-lg shadow-blue-100 active:scale-[0.98]"
              >
                Pay Now
              </button>
            </div>
      ...
      <footer className="sr-only">Payment actions and summary section.</footer>
    </section>
  );
};

// Summary Row Helper
const SummaryRow = ({ icon, label, children, iconBg = "bg-slate-50", iconLabel }) => (
  <div className="flex items-start gap-4">
    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${iconBg}`}>
      <span className="inline-flex" role="img" aria-label={iconLabel}>{icon}</span>
    </div>
