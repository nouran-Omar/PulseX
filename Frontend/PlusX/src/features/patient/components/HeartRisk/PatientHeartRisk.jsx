import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { LuSearch } from 'react-icons/lu';
import { MdOutlineMonitorHeart, MdOutlineSettingsInputAntenna } from 'react-icons/md';
import { LuCloudUpload, LuCircleCheck } from 'react-icons/lu';
import { HiOutlineXMark } from 'react-icons/hi2';
import { LuTriangleAlert } from 'react-icons/lu';
import { IoSearchOutline } from 'react-icons/io5';

/* ─── Upload Card ─── */
const UploadCard = ({ title, desc, Icon, onUpload }) => {
  const [file, setFile] = useState(null);
  const fileRef = React.useRef(null);

  const handleFile = (e) => {
    const f = e.target.files[0];
    if (f) setFile(f);
  };

  const handleDrop = (e) => {
    e.preventDefault();
    const f = e.dataTransfer.files[0];
    if (f) setFile(f);
  };

  return (
    <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex flex-col gap-4">
      {/* header */}
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-[#333CF5] flex items-center justify-center shrink-0">
          <Icon className="text-white text-xl" />
        </div>
        <div>
          <p className="text-sm font-bold text-[#010218]">{title}</p>
          <p className="text-xs text-[#757575]">{desc}</p>
        </div>
      </div>

      {/* drop zone */}
      <div
        onClick={() => fileRef.current.click()}
        onDragOver={(e) => e.preventDefault()}
        onDrop={handleDrop}
        className="border border-[#E5E7EB] rounded-xl bg-white h-[140px] flex flex-col items-center justify-center cursor-pointer hover:border-[#333CF5] transition"
      >
        <div className="w-10 h-10 rounded-full flex items-center justify-center mb-2"
        style={{ background: 'linear-gradient(135deg, #DBEAFE 0%, #BEDBFF 100%)' }}
        >
          <LuCloudUpload className="text-[#155DFC] text-xl" />
        </div>
        <p className="text-xs text-[#364153]">
          Drag & drop {title} files or{' '}
          <span className="text-[#155DFC] font-semibold cursor-pointer">Browse</span>
        </p>
        <p className="text-[10px] text-[#757575] mt-1">Supported formats: JPEG,</p>
        <p className="text-[10px] text-[#757575]">PNG</p>
        <input type="file" ref={fileRef} hidden onChange={handleFile} accept="image/*,.pdf" />
      </div>

      {/* file preview */}
      {file && (
        <div className="flex items-center gap-3 bg-[#F9FAFB] rounded-xl px-4 py-4 border border-[#E5E7EB]">
       <div className='bg-[#DCFCE7] p-2 rounded-lg '>
          <LuCircleCheck className="text-[#00A63E]  text-lg shrink-0" />
         </div> <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-[#010218] truncate">{file.name}</p>
            <p className="text-[10px] text-[#757575]">{(file.size / 1024).toFixed(2)} KB</p>
          </div>
          <button onClick={() => setFile(null)} className="text-gray-400 hover:text-red-500 transition">
            <HiOutlineXMark className="text-base" />
          </button>
        </div>
      )}

      {/* upload button */}
      <button
        disabled={!file}
        onClick={() => file && onUpload(file)}
        className={`w-full py-2.5 rounded-xl text-sm font-semibold transition
          ${file
            ? 'bg-[#333CF5] text-white hover:bg-[#2730d4] shadow-sm'
            : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
      >
        Upload
      </button>
    </div>
  );
};

/* ─── Gauge ─── */
const RiskGauge = ({ percentage = 84 }) => {
  const clamp = Math.min(100, Math.max(0, percentage));
  // semicircle: 0% = -90deg, 100% = 90deg  → rotation = (clamp/100)*180 - 90
  const needleDeg = (clamp / 100) * 180 - 90;

  const color =
    clamp >= 70 ? '#EF4444' :
    clamp >= 40 ? '#F59E0B' :
    '#22C55E';

  const label =
    clamp >= 70 ? 'High Risk' :
    clamp >= 40 ? 'Moderate Risk' :
    'Low Risk';

  const labelColor =
    clamp >= 70 ? 'text-[var(--Black-(Main-Text), #010218)]' :
    clamp >= 40 ? 'text-[var(--Black-(Main-Text), #010218)]' :
    'text-[var(--Black-(Main-Text), #010218)]';

  return (
    <div className="flex  flex-col items-center gap-3 py-8">
      {/* SVG semicircle gauge */}
      <div className="relative w-[180px] h-[100px]">
   <svg viewBox="0 0 180 100" className="w-full h-full" overflow="visible">
      {/* تعريف التدرج اللوني */}
      <defs>
        <linearGradient id="gaugeGradient" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#0913C3" />   {/* اللون الأول (Blue-2) */}
          <stop offset="100%" stopColor="#333CF5" /> {/* اللون الثاني (Blue-1) */}
        </linearGradient>
      </defs>

      {/* القوس الخلفي الرمادي */}
      <path
        d="M 10 90 A 80 80 0 0 1 170 90"
        fill="none" 
        stroke="#E5E7EB" 
        strokeWidth="14" 
        strokeLinecap="round"
      />

      {/* القوس الملون - استدعاء التدرج عن طريق id */}
      <path
        d="M 10 90 A 80 80 0 0 1 170 90"
        fill="none"
        stroke="url(#gaugeGradient)"  // هنا نستخدم التدرج الذي عرفناه فوق
        strokeWidth="14"
        strokeLinecap="round"
        strokeDasharray={`${(clamp / 100) * 251.2} 251.2`}
        className="transition-all duration-500 ease-out" // إضافة حركة ناعمة عند تغير القيمة
      />
    </svg>
        {/* percentage text */}
        <div className="absolute inset-0  flex items-end justify-center pb-1">
          <span className="text-2xl font-bold text-[#010218]">{clamp}%</span>
        </div>
      </div>

      <div className="flex items-center gap-1.5">
        <LuTriangleAlert className={`text-lg text-[#F0B100]`} />
        <span className={`text-sm font-semibold ${labelColor}`}>{label}</span>
      </div>
    </div>
  );
};

/* ─── Critical Alert ─── */
const CriticalAlert = ({ onFindDoctors }) => (
  <div className="rounded-2xl  p-6  flex flex-col items-center text-center gap-3"
  style={{ background: 'linear-gradient(135deg, #FEF2F2 0%, #FFF7ED 100%)' }}
  >
    <div className="w-10 h-10 rounded-full bg-[#FFE2E2] flex items-center justify-center">
      <LuTriangleAlert className="text-[#E7000B] text-xl" />
    </div>
    <h3 className="text-base font-bold text-[#010218]">Critical Alert: Medical Consultation Recommended</h3>
    <p className="text-sm text-[#364153] max-w-sm w-full leading-relaxed">
      Based on our AI analysis of your uploaded X-rays, we have detected indicators that require
      immediate medical attention. Your health is our priority; we have curated a list of top
      cardiologists available on PulseX to help you right now.
    </p>
    <button
      onClick={onFindDoctors}
      className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#333CF5] text-white text-sm font-semibold hover:bg-[#2730d4] transition mt-1"
    >
      Find Doctors <IoSearchOutline className="text-base" />
    </button>
  </div>
);

/* ─── Main Page ─── */
const PatientHeartRisk = () => {
  const navigate = useNavigate();

  const [ecgUploaded,       setEcgUploaded]       = useState(false);
  const [radiologyUploaded, setRadiologyUploaded] = useState(false);
  const [showResult,        setShowResult]         = useState(false);

  const anyUploaded = ecgUploaded || radiologyUploaded;

  // mock result data — replace with API response later
  const mockResult = { percentage: 84, level: 'high' };

  return (
    <div className="min-h-screen   sm:p-6 lg:p-8">

      {/* ── Header ── */}
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          <LuSearch className="text-xl text-[#010218]" />
          <h1 className="text-xl font-bold text-[#010218]">Heart Risk Assessment</h1>
        </div>
        <p className="text-sm text-[#757575]">Upload and view your medical health easily.</p>
      </div>

      {/* ── Upload Cards ── */}
      <div className="grid grid-cols-1 border[#E5E7EB]  sm:grid-cols-2 gap-5">
        <UploadCard
          title="ECG"
          desc="Keep track of your latest ECG results (Optional)."
          Icon={MdOutlineMonitorHeart}
          onUpload={() => setEcgUploaded(true)}
        />
        <UploadCard
          title="Radiology"
          desc="Upload your X-rays or CT(Optional)."
          Icon={MdOutlineSettingsInputAntenna}
          onUpload={() => setRadiologyUploaded(true)}
        />
      </div>

      {/* ── View Result Button ── */}
      {anyUploaded && !showResult && (
        <div className="flex justify-center mt-10">
          <button
            onClick={() => setShowResult(true)}
            className="flex items-center gap-2 px-8 py-5 rounded-full text-white text-sm font-semibold shadow-lg transition hover:opacity-90"
            style={{ background: 'linear-gradient(90deg, var(--Blue-2, #0913C3) 0%, #FF0000 100%)' }}
          >
            <span>🚀</span> View Result
          </button>
        </div>
      )}

      {/* ── Result: Gauge + Critical Alert ── */}
      {showResult && (
        <div className="flex flex-col gap-5 mt-8">
          {/* Gauge card */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-4">
            <RiskGauge percentage={mockResult.percentage} />
          </div>

          {/* Critical alert */}
          {mockResult.level === 'high' && (
            <CriticalAlert onFindDoctors={() => navigate('/patient/doctors')} />
          )}
        </div>
      )}

    </div>
  );
};

export default PatientHeartRisk;

